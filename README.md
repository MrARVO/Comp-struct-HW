...
conda create -n hw_env python=3.10 \--no-default-packages `<br>`{=html}
conda activate hw_env `<br>`{=html}

conda config \--add channels defaults `<br>`{=html} conda config \--add
channels bioconda `<br>`{=html} conda config \--add channels conda-forge
`<br>`{=html}

### install all necessary packages

conda install -y fastqc=0.11.9 `<br>`{=html} conda install -y
star=2.7.10b `<br>`{=html} conda install -y samtools=1.16.1
`<br>`{=html} conda install -y picard `<br>`{=html} conda install -y
salmon=1.9.0 `<br>`{=html} conda install -y bedtools=2.30.0
`<br>`{=html} conda install -y multiqc=1.13 `<br>`{=html}
...
picard - так как цель задания поработать с кондой, то я поставил его
через нее, но там версия 2.27.4 вместо 2.27.5. Можно поставить руками
нужную версию, но тогда она не попадет в conda env, так что я не стал.
Если вдруг это надо было, то на их гитхабе лежит построчный код, как это
поставить руками.

Если какие-то другие пакеты не ставятся, то можно пробовать поменять
канал вручную на bioconda.
